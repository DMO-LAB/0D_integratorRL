    ...
    def collect_optimal_data(self, num_episodes=10, metric='reward', timeout=None):
        ...
        for episode in tqdm(range(num_episodes), desc="Collecting optimal integrator data"):
            ...
            try:
                ...
                end_time = 5 if fixed_temperature <= 1100 else 0.5
...
class Visualizer:
    ...
    def plot_data_collection_stats(stats, save_path=None):
        ...
        if 'episode_stats' in stats and stats['episode_stats']:
            ...
            filtered = [e['filtered_observations'] for e in stats['episode_stats']]
            
            ax2.plot(episodes, filtered, 'r-', label='Filtered')
            ax2.set_title('Observations per Episode')
            ax2.set_xlabel('Episode')
            ax2.set_ylabel('Number of Observations')
            ax2.legend()
            
            # Add total observations text
            if 'filtered_observations' in stats:
                ax2.text(0.5, 0.9, f"Total Observations: {stats['filtered_observations']}", 
                         transform=ax2.transAxes, ha='center',
                         bbox=dict(facecolor='white', alpha=0.8))
        
        # Plot episode computation times
        if 'episode_stats' in stats and stats['episode_stats']:
            times = [e['episode_time'] for e in stats['episode_stats']]
            ax3.plot(episodes, times, 'g-')
            ...
            ax3.axhline(avg_time, color='r', linestyle='--')
            ax3.text(len(episodes)/2, avg_time*1.1, f"Average: {avg_time:.2f}s", ha='center')
        
        # Plot action distribution across episodes
        if 'episode_stats' in stats and stats['episode_stats']:
            # Get all unique actions
            all_actions = set()
            for e in stats['episode_stats']:
                if 'action_counts' in e:
                    all_actions.update(e['action_counts'].keys())
            
            # Create a timeline of action distributions
            for action in all_actions:
                percentages = []
                for e in stats['episode_stats']:
                    if 'action_percentages' in e:
                        percentages.append(e['action_percentages'].get(action, 0))
                    else:
                        percentages.append(0)
                
                ax4.plot(episodes, percentages, label=f'Action {action}')
            
            ax4.set_title('Action Distribution Evolution')
...
def main():
    ...
    if args.collect_data:
        ...
        data_collector = ImprovedIntegratorDataCollector(
